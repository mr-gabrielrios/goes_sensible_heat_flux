...
def temperature_data(query_date, crd, site, bbox, goes_grid_info, nlcd_grid_info, h_0):
    ...
    def crd_to_grid(crd):
        ...
        lat_idx = (np.abs(lat_plot.ravel() - crd[0])).argmin()
        lat_idx = [int(np.floor(lat_idx/lat_plot.shape[1]))-1, (lat_idx % lat_plot.shape[1])]
        lon_idx = (np.abs(lon_plot.ravel() - crd[1])).argmin()
        lon_idx = [int(np.floor(lon_idx/lon_plot.shape[1]))-1, (lon_idx % lon_plot.shape[1])]
    
        if str_switch:
            print('Time: %.4f s' % (time.time()-t))
            
        return [lat_idx[0], lon_idx[1]]
    
    lat_pixel, lon_pixel = crd_to_grid(crd)
